...
type WriterPile struct {
	pile   chan *tar.Writer // channel to receive further items
	list   []*tar.Writer    // list of known items
...
func MakeWriterPile(size, buff int) *WriterPile {
	pile := new(WriterPile)
	pile.list = make([]*tar.Writer, 0, size)
	pile.pile = make(chan *tar.Writer, buff)
	return pile
}
...
func (d *WriterPile) Done() (done <-chan []*tar.Writer) {
	...
	go func(cha chan<- []*tar.Writer, d *WriterPile) {
		defer close(cha)
		d.offset = 0
