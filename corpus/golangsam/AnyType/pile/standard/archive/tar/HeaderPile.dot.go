...
// HeaderPile is a hybrid container for
...
// p.Next() keeps traverses the HeaderPile.
//
...
// MakeHeaderPile returns a (pointer to a) fresh pile
// of items (of type `*tar.Header`)
...
func (d *HeaderPile) Done() (done <-chan []*tar.Header) {
	...
	go func(cha chan<- []*tar.Header, d *HeaderPile) {
		defer close(cha)
		d.offset = 0
