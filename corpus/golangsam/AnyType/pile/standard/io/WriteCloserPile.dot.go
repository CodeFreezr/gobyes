...
// WriteCloserPile is a hybrid container for
...
// p.Next() keeps traverses the WriteCloserPile.
//
...
// MakeWriteCloserPile returns a (pointer to a) fresh pile
// of items (of type `io.WriteCloser`)
...
func (d *WriteCloserPile) Done() (done <-chan []io.WriteCloser) {
	...
	go func(cha chan<- []io.WriteCloser, d *WriteCloserPile) {
		defer close(cha)
		d.offset = 0
