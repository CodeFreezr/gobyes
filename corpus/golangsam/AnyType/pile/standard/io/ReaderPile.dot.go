...
type ReaderPile struct {
	pile   chan io.Reader // channel to receive further items
	list   []io.Reader    // list of known items
...
func MakeReaderPile(size, buff int) *ReaderPile {
	pile := new(ReaderPile)
	pile.list = make([]io.Reader, 0, size)
	pile.pile = make(chan io.Reader, buff)
	return pile
}
...
func (d *ReaderPile) Done() (done <-chan []io.Reader) {
	...
	go func(cha chan<- []io.Reader, d *ReaderPile) {
		defer close(cha)
		d.offset = 0
