...
// WriteSeekerPile is a hybrid container for
// a lazily and concurrently populated growing-only slice
// of items (of type `io.WriteSeeker`)
...
// p.Next() keeps traverses the WriteSeekerPile.
//
...
type WriteSeekerPile struct {
	pile   chan io.WriteSeeker // channel to receive further items
	list   []io.WriteSeeker    // list of known items
	offset int                 // index for Next()
}

// MakeWriteSeekerPile returns a (pointer to a) fresh pile
// of items (of type `io.WriteSeeker`)
...
func MakeWriteSeekerPile(size, buff int) *WriteSeekerPile {
	pile := new(WriteSeekerPile)
	pile.list = make([]io.WriteSeeker, 0, size)
	pile.pile = make(chan io.WriteSeeker, buff)
	return pile
}
...
func (d *WriteSeekerPile) Done() (done <-chan []io.WriteSeeker) {
	...
	go func(cha chan<- []io.WriteSeeker, d *WriteSeekerPile) {
		defer close(cha)
		d.offset = 0
