...
// ReadWriteSeekerPile is a hybrid container for
...
// p.Next() keeps traverses the ReadWriteSeekerPile.
//
...
// MakeReadWriteSeekerPile returns a (pointer to a) fresh pile
// of items (of type `io.ReadWriteSeeker`)
...
func (d *ReadWriteSeekerPile) Done() (done <-chan []io.ReadWriteSeeker) {
	...
	go func(cha chan<- []io.ReadWriteSeeker, d *ReadWriteSeekerPile) {
		defer close(cha)
		d.offset = 0
