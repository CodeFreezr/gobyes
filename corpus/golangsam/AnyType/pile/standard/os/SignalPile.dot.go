...
// SignalPile is a hybrid container for
...
// p.Next() keeps traverses the SignalPile.
//
...
// MakeSignalPile returns a (pointer to a) fresh pile
// of items (of type `os.Signal`)
...
func (d *SignalPile) Done() (done <-chan []os.Signal) {
	...
	go func(cha chan<- []os.Signal, d *SignalPile) {
		defer close(cha)
		d.offset = 0
