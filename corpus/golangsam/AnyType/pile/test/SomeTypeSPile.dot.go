...
// SomeTypeSPile is a hybrid container for
...
//
// Usage for a pile `p`:
//  p := MakeSomeTypeSPile(128, 32)
//
// Have it grow concurrently using multiple:
//  var item []SomeType = something
//  p.Pile(item)
...
// p.Next() keeps traverses the SomeTypeSPile.
//
...
func (d *SomeTypeSPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *SomeTypeSPile) Iter() (item []SomeType, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *SomeTypeSPile) Next() (item []SomeType, ok bool) {
...
// Done returns a channel which emits the result (as slice of SomeTypeS) once the pile is closed.
//
...
func (d *SomeTypeSPile) Done() (done <-chan [][]SomeType) {
	cha := make(chan [][]SomeType)
	go func(cha chan<- [][]SomeType, d *SomeTypeSPile) {
		defer close(cha)
		d.offset = 0
