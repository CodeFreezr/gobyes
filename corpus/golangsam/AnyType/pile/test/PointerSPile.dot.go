...
// PointerSPile is a hybrid container for
...
// p.Next() keeps traverses the PointerSPile.
//
...
// Pile appends an `[]*SomeType` item to the PointerSPile.
//
...
func (d *PointerSPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *PointerSPile) Iter() (item []*SomeType, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of PointerS) once the pile is closed.
//
...
func (d *PointerSPile) Done() (done <-chan [][]*SomeType) {
	cha := make(chan [][]*SomeType)
	go func(cha chan<- [][]*SomeType, d *PointerSPile) {
		defer close(cha)
		d.offset = 0
