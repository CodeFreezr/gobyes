...
// DotPathPile is a hybrid container for
...
//  var item *dotpath.DotPath = something
//  p.Pile(item)
...
// p.Next() keeps traverses the DotPathPile.
//
...
func (d *DotPathPile) Done() (done <-chan []*dotpath.DotPath) {
	...
	go func(cha chan<- []*dotpath.DotPath, d *DotPathPile) {
		defer close(cha)
		d.offset = 0
