...
// p.Next() keeps traverses the LSMPile.
//
...
//
type LSMPile struct {
	pile   chan *lsm.LazyStringerMap // channel to receive further items
	list   []*lsm.LazyStringerMap    // list of known items
	offset int                       // index for Next()
}

// MakeLSMPile returns a (pointer to a) fresh pile
// of items (of type `*lsm.LazyStringerMap`)
// with size as initial capacity
// and
// with buff as initial leeway, allowing as many Pile's to execute non-blocking before respective Done or Next's.
func MakeLSMPile(size, buff int) *LSMPile {
	pile := new(LSMPile)
	pile.list = make([]*lsm.LazyStringerMap, 0, size)
	pile.pile = make(chan *lsm.LazyStringerMap, buff)
	return pile
}

// Pile appends an `*lsm.LazyStringerMap` item to the LSMPile.
//
// Note: Pile will block iff buff is exceeded and no Done() or Next()'s are used.
func (d *LSMPile) Pile(item *lsm.LazyStringerMap) {
	d.pile <- item
}
...
func (d *LSMPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *LSMPile) Iter() (item *lsm.LazyStringerMap, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *LSMPile) Next() (item *lsm.LazyStringerMap, ok bool) {
...
// Done returns a channel which emits the result (as slice of LSM) once the pile is closed.
//
...
func (d *LSMPile) Done() (done <-chan []*lsm.LazyStringerMap) {
	cha := make(chan []*lsm.LazyStringerMap)
	go func(cha chan<- []*lsm.LazyStringerMap, d *LSMPile) {
		defer close(cha)
		d.offset = 0
