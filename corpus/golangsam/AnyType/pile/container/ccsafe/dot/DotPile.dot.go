...
// DotPile is a hybrid container for
...
//
// Usage for a pile `p`:
//  p := MakeDotPile(128, 32)
//
// Have it grow concurrently using multiple:
//  var item *dot.Dot = something
//  p.Pile(item)
...
// p.Next() keeps traverses the DotPile.
//
...
// Pile appends an `*dot.Dot` item to the DotPile.
//
...
func (d *DotPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *DotPile) Iter() (item *dot.Dot, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of Dot) once the pile is closed.
//
...
func (d *DotPile) Done() (done <-chan []*dot.Dot) {
	cha := make(chan []*dot.Dot)
	go func(cha chan<- []*dot.Dot, d *DotPile) {
		defer close(cha)
		d.offset = 0
