...
// TagPile is a hybrid container for
...
//
// Usage for a pile `p`:
//  p := MakeTagPile(128, 32)
//
// Have it grow concurrently using multiple:
//  var item *tag.TagAny = something
//  p.Pile(item)
...
// p.Next() keeps traverses the TagPile.
//
...
// Pile appends an `*tag.TagAny` item to the TagPile.
//
...
func (d *TagPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *TagPile) Iter() (item *tag.TagAny, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of Tag) once the pile is closed.
//
...
func (d *TagPile) Done() (done <-chan []*tag.TagAny) {
	cha := make(chan []*tag.TagAny)
	go func(cha chan<- []*tag.TagAny, d *TagPile) {
		defer close(cha)
		d.offset = 0
