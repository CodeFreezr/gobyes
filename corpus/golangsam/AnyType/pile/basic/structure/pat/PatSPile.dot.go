...
// p.Next() keeps traverses the PatSPile.
//
...
//
type PatSPile struct {
	pile   chan []struct{} // channel to receive further items
	list   [][]struct{}    // list of known items
	offset int             // index for Next()
}

// MakePatSPile returns a (pointer to a) fresh pile
// of items (of type `[]struct{}`)
// with size as initial capacity
// and
// with buff as initial leeway, allowing as many Pile's to execute non-blocking before respective Done or Next's.
func MakePatSPile(size, buff int) *PatSPile {
	pile := new(PatSPile)
	pile.list = make([][]struct{}, 0, size)
	pile.pile = make(chan []struct{}, buff)
	return pile
}

// Pile appends an `[]struct{}` item to the PatSPile.
//
// Note: Pile will block iff buff is exceeded and no Done() or Next()'s are used.
func (d *PatSPile) Pile(item []struct{}) {
	d.pile <- item
}
...
func (d *PatSPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *PatSPile) Iter() (item []struct{}, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *PatSPile) Next() (item []struct{}, ok bool) {
...
// Done returns a channel which emits the result (as slice of PatS) once the pile is closed.
//
...
func (d *PatSPile) Done() (done <-chan [][]struct{}) {
	cha := make(chan [][]struct{})
	go func(cha chan<- [][]struct{}, d *PatSPile) {
		defer close(cha)
		d.offset = 0
