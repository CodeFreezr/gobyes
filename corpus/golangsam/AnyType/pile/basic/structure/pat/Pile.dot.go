...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *Pile) Iter() (item struct{}, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *Pile) Next() (item struct{}, ok bool) {
...
func (d *Pile) Done() (done <-chan []struct{}) {
	cha := make(chan []struct{})
	go func(cha chan<- []struct{}, d *Pile) {
		defer close(cha)
		d.offset = 0
