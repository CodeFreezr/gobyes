...
// Pile appends an `interface{}` item to the Pile.
//
// Note: Pile will block iff buff is exceeded and no Done() or Next()'s are used.
func (d *Pile) Pile(item interface{}) {
	d.pile <- item
}
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *Pile) Iter() (item interface{}, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *Pile) Next() (item interface{}, ok bool) {
...
func (d *Pile) Done() (done <-chan []interface{}) {
	...
	go func(cha chan<- []interface{}, d *Pile) {
		defer close(cha)
		d.offset = 0
