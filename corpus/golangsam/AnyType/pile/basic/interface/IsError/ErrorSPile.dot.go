...
// p.Next() keeps traverses the ErrorSPile.
//
...
//
type ErrorSPile struct {
	pile   chan []error // channel to receive further items
	list   [][]error    // list of known items
	offset int          // index for Next()
}

// MakeErrorSPile returns a (pointer to a) fresh pile
// of items (of type `[]error`)
// with size as initial capacity
// and
// with buff as initial leeway, allowing as many Pile's to execute non-blocking before respective Done or Next's.
func MakeErrorSPile(size, buff int) *ErrorSPile {
	pile := new(ErrorSPile)
	pile.list = make([][]error, 0, size)
	pile.pile = make(chan []error, buff)
	return pile
}

// Pile appends an `[]error` item to the ErrorSPile.
//
// Note: Pile will block iff buff is exceeded and no Done() or Next()'s are used.
func (d *ErrorSPile) Pile(item []error) {
	d.pile <- item
}
...
func (d *ErrorSPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *ErrorSPile) Iter() (item []error, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *ErrorSPile) Next() (item []error, ok bool) {
...
// Done returns a channel which emits the result (as slice of ErrorS) once the pile is closed.
//
...
func (d *ErrorSPile) Done() (done <-chan [][]error) {
	cha := make(chan [][]error)
	go func(cha chan<- [][]error, d *ErrorSPile) {
		defer close(cha)
		d.offset = 0
