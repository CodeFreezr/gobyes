...
// Int32Pile is a hybrid container for
...
//
// Usage for a pile `p`:
//  p := MakeInt32Pile(128, 32)
//
// Have it grow concurrently using multiple:
//  var item int32 = something
//  p.Pile(item)
...
// p.Next() keeps traverses the Int32Pile.
//
...
// Pile appends an `int32` item to the Int32Pile.
//
...
func (d *Int32Pile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *Int32Pile) Iter() (item int32, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of Int32) once the pile is closed.
//
...
func (d *Int32Pile) Done() (done <-chan []int32) {
	cha := make(chan []int32)
	go func(cha chan<- []int32, d *Int32Pile) {
		defer close(cha)
		d.offset = 0
