...
// StringSPile is a hybrid container for
...
// p.Next() keeps traverses the StringSPile.
//
...
// Pile appends an `[]string` item to the StringSPile.
//
...
func (d *StringSPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *StringSPile) Iter() (item []string, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of StringS) once the pile is closed.
//
...
func (d *StringSPile) Done() (done <-chan [][]string) {
	cha := make(chan [][]string)
	go func(cha chan<- [][]string, d *StringSPile) {
		defer close(cha)
		d.offset = 0
