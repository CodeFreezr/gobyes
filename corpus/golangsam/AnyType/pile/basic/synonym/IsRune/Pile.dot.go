...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *Pile) Iter() (item rune, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *Pile) Next() (item rune, ok bool) {
...
func (d *Pile) Done() (done <-chan []rune) {
	cha := make(chan []rune)
	go func(cha chan<- []rune, d *Pile) {
		defer close(cha)
		d.offset = 0
