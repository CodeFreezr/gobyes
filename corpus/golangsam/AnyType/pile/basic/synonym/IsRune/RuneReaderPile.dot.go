...
// RuneReaderPile is a hybrid container for
// a lazily and concurrently populated growing-only slice
// of items (of type `io.RuneReader`)
...
// p.Next() keeps traverses the RuneReaderPile.
//
...
type RuneReaderPile struct {
	pile   chan io.RuneReader // channel to receive further items
	list   []io.RuneReader    // list of known items
	offset int                // index for Next()
}

// MakeRuneReaderPile returns a (pointer to a) fresh pile
// of items (of type `io.RuneReader`)
...
func MakeRuneReaderPile(size, buff int) *RuneReaderPile {
	pile := new(RuneReaderPile)
	pile.list = make([]io.RuneReader, 0, size)
	pile.pile = make(chan io.RuneReader, buff)
	return pile
}
...
func (d *RuneReaderPile) Done() (done <-chan []io.RuneReader) {
	...
	go func(cha chan<- []io.RuneReader, d *RuneReaderPile) {
		defer close(cha)
		d.offset = 0
