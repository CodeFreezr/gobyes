...
// ByteWriterPile is a hybrid container for
// a lazily and concurrently populated growing-only slice
// of items (of type `io.ByteWriter`)
...
// p.Next() keeps traverses the ByteWriterPile.
//
...
type ByteWriterPile struct {
	pile   chan io.ByteWriter // channel to receive further items
	list   []io.ByteWriter    // list of known items
	offset int                // index for Next()
}

// MakeByteWriterPile returns a (pointer to a) fresh pile
// of items (of type `io.ByteWriter`)
...
func MakeByteWriterPile(size, buff int) *ByteWriterPile {
	pile := new(ByteWriterPile)
	pile.list = make([]io.ByteWriter, 0, size)
	pile.pile = make(chan io.ByteWriter, buff)
	return pile
}
...
func (d *ByteWriterPile) Done() (done <-chan []io.ByteWriter) {
	...
	go func(cha chan<- []io.ByteWriter, d *ByteWriterPile) {
		defer close(cha)
		d.offset = 0
