...
// BytePile is a hybrid container for
...
//
// Usage for a pile `p`:
//  p := MakeBytePile(128, 32)
//
// Have it grow concurrently using multiple:
//  var item byte = something
//  p.Pile(item)
...
// p.Next() keeps traverses the BytePile.
//
...
// Pile appends an `byte` item to the BytePile.
//
...
func (d *BytePile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *BytePile) Iter() (item byte, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of Byte) once the pile is closed.
//
...
func (d *BytePile) Done() (done <-chan []byte) {
	cha := make(chan []byte)
	go func(cha chan<- []byte, d *BytePile) {
		defer close(cha)
		d.offset = 0
