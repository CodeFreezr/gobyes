...
func JoinComplex64Chan(out chan<- complex64, inp <-chan complex64) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, out chan<- complex64, inp <-chan complex64) {
		defer close(done)
		for i := range inp {
			out <- i
		}
...
func DoneComplex64(inp <-chan complex64) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, inp <-chan complex64) {
		defer close(done)
		for i := range inp {
			_ = i // Drain inp
		}
...
func DoneComplex64Slice(inp <-chan complex64) (done <-chan []complex64) {
	cha := make(chan []complex64)
	go func(inp <-chan complex64, done chan<- []complex64) {
		defer close(done)
...
func DoneComplex64Func(inp <-chan complex64, act func(a complex64)) (out <-chan struct{}) {
	...
	go func(done chan<- struct{}, inp <-chan complex64, act func(a complex64)) {
		defer close(done)
		for i := range inp {
			act(i) // Apply action
		}
...
func PipeComplex64Buffer(inp <-chan complex64, cap int) (out <-chan complex64) {
	...
	go func(out chan<- complex64, inp <-chan complex64) {
		defer close(out)
		for i := range inp {
			out <- i
		}
...
func PipeComplex64Func(inp <-chan complex64, act func(a complex64) complex64) (out <-chan complex64) {
	...
	go func(out chan<- complex64, inp <-chan complex64, act func(a complex64) complex64) {
		defer close(out)
		for i := range inp {
			out <- act(i)
		}
...
func PipeComplex64Fork(inp <-chan complex64) (out1, out2 <-chan complex64) {
	...
	go func(out1, out2 chan<- complex64, inp <-chan complex64) {
		defer close(out1)
		defer close(out2)
		for i := range inp {
			out1 <- i
