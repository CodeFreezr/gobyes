...
// Note: SendProxyHeader imports "container/ring" for the expanding buffer.
import (
	tar "archive/tar"
	"container/ring"
)
...
// SendProxyHeader returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyHeader(out chan<- *tar.Header) chan<- *tar.Header {
	proxy := make(chan *tar.Header, HeaderCAP)
	go func() {
		n := HeaderQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- *tar.Header
		var e *tar.Header
		for {
			c = out
			if first == last {
