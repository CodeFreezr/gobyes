...
func DoneSplitFunc(inp <-chan bufio.SplitFunc) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, inp <-chan bufio.SplitFunc) {
		defer close(done)
		for i := range inp {
			_ = i // Drain inp
		}
...
func DoneSplitFuncSlice(inp <-chan bufio.SplitFunc) (done <-chan []bufio.SplitFunc) {
	cha := make(chan []bufio.SplitFunc)
	go func(inp <-chan bufio.SplitFunc, done chan<- []bufio.SplitFunc) {
		defer close(done)
...
func DoneSplitFuncFunc(inp <-chan bufio.SplitFunc, act func(a bufio.SplitFunc)) (out <-chan struct{}) {
	...
	go func(done chan<- struct{}, inp <-chan bufio.SplitFunc, act func(a bufio.SplitFunc)) {
		defer close(done)
		for i := range inp {
			act(i) // Apply action
		}
...
func PipeSplitFuncBuffer(inp <-chan bufio.SplitFunc, cap int) (out <-chan bufio.SplitFunc) {
	...
	go func(out chan<- bufio.SplitFunc, inp <-chan bufio.SplitFunc) {
		defer close(out)
		for i := range inp {
			out <- i
		}
...
func PipeSplitFuncFunc(inp <-chan bufio.SplitFunc, act func(a bufio.SplitFunc) bufio.SplitFunc) (out <-chan bufio.SplitFunc) {
	...
	go func(out chan<- bufio.SplitFunc, inp <-chan bufio.SplitFunc, act func(a bufio.SplitFunc) bufio.SplitFunc) {
		defer close(out)
		for i := range inp {
			out <- act(i)
		}
...
func PipeSplitFuncFork(inp <-chan bufio.SplitFunc) (out1, out2 <-chan bufio.SplitFunc) {
	...
	go func(out1, out2 chan<- bufio.SplitFunc, inp <-chan bufio.SplitFunc) {
		defer close(out1)
		defer close(out2)
		for i := range inp {
			out1 <- i
