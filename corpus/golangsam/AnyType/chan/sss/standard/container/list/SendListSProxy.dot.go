...
// Note: SendProxyListS imports "container/ring" for the expanding buffer.
import (
	list "container/list"
	"container/ring"
)
...
// SendProxyListS returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyListS(out chan<- []*list.List) chan<- []*list.List {
	proxy := make(chan []*list.List, ListSCAP)
	go func() {
		n := ListSQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- []*list.List
		var e []*list.List
		for {
			c = out
			if first == last {
