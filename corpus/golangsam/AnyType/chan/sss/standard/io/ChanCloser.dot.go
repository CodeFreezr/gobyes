...
func ChanCloser(inp ...io.Closer) (out <-chan io.Closer) {
	...
	go func(out chan<- io.Closer, inp ...io.Closer) {
		defer close(out)
		for i := range inp {
			out <- inp[i]
		}
...
func ChanCloserFuncNok(act func() (io.Closer, bool)) (out <-chan io.Closer) {
	cha := make(chan io.Closer)
	go func(out chan<- io.Closer, act func() (io.Closer, bool)) {
		defer close(out)
		for {
...
func ChanCloserFuncErr(act func() (io.Closer, error)) (out <-chan io.Closer) {
	cha := make(chan io.Closer)
	go func(out chan<- io.Closer, act func() (io.Closer, error)) {
		defer close(out)
		for {
...
func JoinCloser(out chan<- io.Closer, inp ...io.Closer) (done <-chan struct{}) {
	...
	go func(done chan<- struct{}, out chan<- io.Closer, inp ...io.Closer) {
		defer close(done)
		for i := range inp {
			out <- inp[i]
		}
...
func JoinCloserChan(out chan<- io.Closer, inp <-chan io.Closer) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, out chan<- io.Closer, inp <-chan io.Closer) {
		defer close(done)
		for i := range inp {
			out <- i
		}
...
func DoneCloser(inp <-chan io.Closer) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, inp <-chan io.Closer) {
		defer close(done)
		for i := range inp {
			_ = i // Drain inp
		}
...
func DoneCloserSlice(inp <-chan io.Closer) (done <-chan []io.Closer) {
	cha := make(chan []io.Closer)
	go func(inp <-chan io.Closer, done chan<- []io.Closer) {
		defer close(done)
		CloserS := []io.Closer{}
		for i := range inp {
...
func DoneCloserFunc(inp <-chan io.Closer, act func(a io.Closer)) (out <-chan struct{}) {
	...
	go func(done chan<- struct{}, inp <-chan io.Closer, act func(a io.Closer)) {
		defer close(done)
		for i := range inp {
			act(i) // Apply action
		}
...
func PipeCloserBuffer(inp <-chan io.Closer, cap int) (out <-chan io.Closer) {
	...
	go func(out chan<- io.Closer, inp <-chan io.Closer) {
		defer close(out)
		for i := range inp {
			out <- i
		}
...
func PipeCloserFunc(inp <-chan io.Closer, act func(a io.Closer) io.Closer) (out <-chan io.Closer) {
	...
	go func(out chan<- io.Closer, inp <-chan io.Closer, act func(a io.Closer) io.Closer) {
		defer close(out)
		for i := range inp {
			out <- act(i)
		}
...
func PipeCloserFork(inp <-chan io.Closer) (out1, out2 <-chan io.Closer) {
	...
	go func(out1, out2 chan<- io.Closer, inp <-chan io.Closer) {
		defer close(out1)
		defer close(out2)
		for i := range inp {
			out1 <- i
