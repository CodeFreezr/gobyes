...
func ChanReadCloser(inp ...io.ReadCloser) (out <-chan io.ReadCloser) {
	...
	go func(out chan<- io.ReadCloser, inp ...io.ReadCloser) {
		defer close(out)
		for i := range inp {
			out <- inp[i]
		}
...
func ChanReadCloserFuncNok(act func() (io.ReadCloser, bool)) (out <-chan io.ReadCloser) {
	cha := make(chan io.ReadCloser)
	go func(out chan<- io.ReadCloser, act func() (io.ReadCloser, bool)) {
		defer close(out)
		for {
...
func ChanReadCloserFuncErr(act func() (io.ReadCloser, error)) (out <-chan io.ReadCloser) {
	cha := make(chan io.ReadCloser)
	go func(out chan<- io.ReadCloser, act func() (io.ReadCloser, error)) {
		defer close(out)
		for {
...
func DoneReadCloser(inp <-chan io.ReadCloser) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, inp <-chan io.ReadCloser) {
		defer close(done)
		for i := range inp {
			_ = i // Drain inp
		}
...
func DoneReadCloserSlice(inp <-chan io.ReadCloser) (done <-chan []io.ReadCloser) {
	cha := make(chan []io.ReadCloser)
	go func(inp <-chan io.ReadCloser, done chan<- []io.ReadCloser) {
		defer close(done)
...
func DoneReadCloserFunc(inp <-chan io.ReadCloser, act func(a io.ReadCloser)) (out <-chan struct{}) {
	...
	go func(done chan<- struct{}, inp <-chan io.ReadCloser, act func(a io.ReadCloser)) {
		defer close(done)
		for i := range inp {
			act(i) // Apply action
		}
...
func PipeReadCloserBuffer(inp <-chan io.ReadCloser, cap int) (out <-chan io.ReadCloser) {
	...
	go func(out chan<- io.ReadCloser, inp <-chan io.ReadCloser) {
		defer close(out)
		for i := range inp {
			out <- i
		}
...
func PipeReadCloserFunc(inp <-chan io.ReadCloser, act func(a io.ReadCloser) io.ReadCloser) (out <-chan io.ReadCloser) {
	...
	go func(out chan<- io.ReadCloser, inp <-chan io.ReadCloser, act func(a io.ReadCloser) io.ReadCloser) {
		defer close(out)
		for i := range inp {
			out <- act(i)
		}
...
func PipeReadCloserFork(inp <-chan io.ReadCloser) (out1, out2 <-chan io.ReadCloser) {
	...
	go func(out1, out2 chan<- io.ReadCloser, inp <-chan io.ReadCloser) {
		defer close(out1)
		defer close(out2)
		for i := range inp {
			out1 <- i
