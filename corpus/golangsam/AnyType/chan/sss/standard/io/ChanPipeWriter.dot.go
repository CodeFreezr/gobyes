...
func ChanPipeWriter(inp ...*io.PipeWriter) (out <-chan *io.PipeWriter) {
	...
	go func(out chan<- *io.PipeWriter, inp ...*io.PipeWriter) {
		defer close(out)
		for i := range inp {
			out <- inp[i]
		}
...
func ChanPipeWriterFuncNok(act func() (*io.PipeWriter, bool)) (out <-chan *io.PipeWriter) {
	cha := make(chan *io.PipeWriter)
	go func(out chan<- *io.PipeWriter, act func() (*io.PipeWriter, bool)) {
		defer close(out)
		for {
...
func ChanPipeWriterFuncErr(act func() (*io.PipeWriter, error)) (out <-chan *io.PipeWriter) {
	cha := make(chan *io.PipeWriter)
	go func(out chan<- *io.PipeWriter, act func() (*io.PipeWriter, error)) {
		defer close(out)
		for {
...
func DonePipeWriter(inp <-chan *io.PipeWriter) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, inp <-chan *io.PipeWriter) {
		defer close(done)
		for i := range inp {
			_ = i // Drain inp
		}
...
func DonePipeWriterSlice(inp <-chan *io.PipeWriter) (done <-chan []*io.PipeWriter) {
	cha := make(chan []*io.PipeWriter)
	go func(inp <-chan *io.PipeWriter, done chan<- []*io.PipeWriter) {
		defer close(done)
...
func DonePipeWriterFunc(inp <-chan *io.PipeWriter, act func(a *io.PipeWriter)) (out <-chan struct{}) {
	...
	go func(done chan<- struct{}, inp <-chan *io.PipeWriter, act func(a *io.PipeWriter)) {
		defer close(done)
		for i := range inp {
			act(i) // Apply action
		}
...
func PipePipeWriterBuffer(inp <-chan *io.PipeWriter, cap int) (out <-chan *io.PipeWriter) {
	...
	go func(out chan<- *io.PipeWriter, inp <-chan *io.PipeWriter) {
		defer close(out)
		for i := range inp {
			out <- i
		}
...
func PipePipeWriterFunc(inp <-chan *io.PipeWriter, act func(a *io.PipeWriter) *io.PipeWriter) (out <-chan *io.PipeWriter) {
	...
	go func(out chan<- *io.PipeWriter, inp <-chan *io.PipeWriter, act func(a *io.PipeWriter) *io.PipeWriter) {
		defer close(out)
		for i := range inp {
			out <- act(i)
		}
...
func PipePipeWriterFork(inp <-chan *io.PipeWriter) (out1, out2 <-chan *io.PipeWriter) {
	...
	go func(out1, out2 chan<- *io.PipeWriter, inp <-chan *io.PipeWriter) {
		defer close(out1)
		defer close(out2)
		for i := range inp {
			out1 <- i
