...
func chanReadCloserFuncNil(out chan<- *zip.ReadCloser, act func() *zip.ReadCloser) {
	defer close(out)
	for {
...
func ChanReadCloserFuncNil(act func() *zip.ReadCloser) (out <-chan *zip.ReadCloser) {
	cha := make(chan *zip.ReadCloser)
	go chanReadCloserFuncNil(cha, act)
	return cha
}

func chanReadCloserFuncNok(out chan<- *zip.ReadCloser, act func() (*zip.ReadCloser, bool)) {
	defer close(out)
	for {
...
func chanReadCloserFuncErr(out chan<- *zip.ReadCloser, act func() (*zip.ReadCloser, error)) {
	defer close(out)
	for {
