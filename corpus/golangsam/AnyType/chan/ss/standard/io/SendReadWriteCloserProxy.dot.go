...
// Note: SendProxyReadWriteCloser imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyReadWriteCloser returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyReadWriteCloser(out chan<- io.ReadWriteCloser) chan<- io.ReadWriteCloser {
	proxy := make(chan io.ReadWriteCloser, ReadWriteCloserCAP)
	go func() {
		n := ReadWriteCloserQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.ReadWriteCloser
		var e io.ReadWriteCloser
		for {
			c = out
			if first == last {
				...
				e = first.Value.(io.ReadWriteCloser)
			}
			select {
			case e = <-proxy:
				last.Value = e
