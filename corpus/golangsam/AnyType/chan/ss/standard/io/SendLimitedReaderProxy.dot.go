...
// Note: SendProxyLimitedReader imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyLimitedReader returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyLimitedReader(out chan<- *io.LimitedReader) chan<- *io.LimitedReader {
	proxy := make(chan *io.LimitedReader, LimitedReaderCAP)
	go func() {
		n := LimitedReaderQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- *io.LimitedReader
		var e *io.LimitedReader
		for {
			c = out
			if first == last {
				...
				e = first.Value.(*io.LimitedReader)
			}
			select {
			case e = <-proxy:
				last.Value = e
