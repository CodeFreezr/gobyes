...
// Note: SendProxyWriteSeeker imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyWriteSeeker returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyWriteSeeker(out chan<- io.WriteSeeker) chan<- io.WriteSeeker {
	proxy := make(chan io.WriteSeeker, WriteSeekerCAP)
	go func() {
		n := WriteSeekerQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.WriteSeeker
		var e io.WriteSeeker
		for {
			c = out
			if first == last {
