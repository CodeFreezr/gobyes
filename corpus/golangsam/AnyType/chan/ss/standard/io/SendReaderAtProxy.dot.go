...
// Note: SendProxyReaderAt imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyReaderAt returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyReaderAt(out chan<- io.ReaderAt) chan<- io.ReaderAt {
	proxy := make(chan io.ReaderAt, ReaderAtCAP)
	go func() {
		n := ReaderAtQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.ReaderAt
		var e io.ReaderAt
		for {
			c = out
			if first == last {
