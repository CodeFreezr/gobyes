...
func chanUInt8FuncNok(out chan<- uint8, act func() (uint8, bool)) {
	defer close(out)
	for {
...
func ChanUInt8FuncNok(act func() (uint8, bool)) (out <-chan uint8) {
	cha := make(chan uint8)
	go chanUInt8FuncNok(cha, act)
	return cha
}

func chanUInt8FuncErr(out chan<- uint8, act func() (uint8, error)) {
	defer close(out)
	for {
