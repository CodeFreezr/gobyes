...
func JoinRuneSSlice(out chan<- []rune, inp ...[][]rune) (done <-chan struct{}) {
	cha := make(chan struct{})
	go joinRuneSSlice(cha, out, inp...)
	return cha
}

func joinRuneSChan(done chan<- struct{}, out chan<- []rune, inp <-chan []rune) {
	defer close(done)
	for i := range inp {
		out <- i
	}
...
func JoinRuneSChan(out chan<- []rune, inp <-chan []rune) (done <-chan struct{}) {
	cha := make(chan struct{})
	go joinRuneSChan(cha, out, inp)
	return cha
}

func doitRuneS(done chan<- struct{}, inp <-chan []rune) {
	defer close(done)
	for i := range inp {
		_ = i // Drain inp
	}
...
func DoneRuneS(inp <-chan []rune) (done <-chan struct{}) {
	cha := make(chan struct{})
	go doitRuneS(cha, inp)
	return cha
}

func doitRuneSSlice(done chan<- ([][]rune), inp <-chan []rune) {
	defer close(done)
	RuneSS := [][]rune{}
	for i := range inp {
...
func DoneRuneSSlice(inp <-chan []rune) (done <-chan ([][]rune)) {
	cha := make(chan ([][]rune))
	go doitRuneSSlice(cha, inp)
	return cha
}

func doitRuneSFunc(done chan<- struct{}, inp <-chan []rune, act func(a []rune)) {
	defer close(done)
	for i := range inp {
		act(i) // Apply action
	}
...
func DoneRuneSFunc(inp <-chan []rune, act func(a []rune)) (out <-chan struct{}) {
	...
	go doitRuneSFunc(cha, inp, act)
	return cha
}

func pipeRuneSBuffer(out chan<- []rune, inp <-chan []rune) {
	defer close(out)
	for i := range inp {
		out <- i
	}
...
func PipeRuneSBuffer(inp <-chan []rune, cap int) (out <-chan []rune) {
	...
	go pipeRuneSBuffer(cha, inp)
	return cha
}

func pipeRuneSFunc(out chan<- []rune, inp <-chan []rune, act func(a []rune) []rune) {
	defer close(out)
	for i := range inp {
		out <- act(i)
	}
...
func PipeRuneSFunc(inp <-chan []rune, act func(a []rune) []rune) (out <-chan []rune) {
	...
	go pipeRuneSFunc(cha, inp, act)
	return cha
}

func pipeRuneSFork(out1, out2 chan<- []rune, inp <-chan []rune) {
	defer close(out1)
	defer close(out2)
	for i := range inp {
		out1 <- i
