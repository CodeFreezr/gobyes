...
// Note: SendProxyUInt64 imports "container/ring" for the expanding buffer.
import (
	"container/ring"
)
...
// SendProxyUInt64 returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyUInt64(out chan<- uint64) chan<- uint64 {
	proxy := make(chan uint64, UInt64CAP)
	go func() {
		n := UInt64QUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- uint64
		var e uint64
