...
// Note: SendProxyInt imports "container/ring" for the expanding buffer.
import (
	"container/ring"
)
...
// SendProxyInt returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyInt(out chan<- int) chan<- int {
	proxy := make(chan int, IntCAP)
	go func() {
		n := IntQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- int
		var e int
