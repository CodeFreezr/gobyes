...
// Note: SendProxyPatS imports "container/ring" for the expanding buffer.
import (
	"container/ring"
)
...
// SendProxyPatS returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyPatS(out chan<- []struct{}) chan<- []struct{} {
	proxy := make(chan []struct{}, PatSCAP)
	go func() {
		n := PatSQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- []struct{}
		var e []struct{}
