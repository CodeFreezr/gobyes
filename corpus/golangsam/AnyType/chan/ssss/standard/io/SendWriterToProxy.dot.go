...
// Note: SendProxyWriterTo imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyWriterTo returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyWriterTo(out chan<- io.WriterTo) chan<- io.WriterTo {
	proxy := make(chan io.WriterTo, WriterToCAP)
	go func() {
		n := WriterToQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.WriterTo
		var e io.WriterTo
		for {
			c = out
			if first == last {
