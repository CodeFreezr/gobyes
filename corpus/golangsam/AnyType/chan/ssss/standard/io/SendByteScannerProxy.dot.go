...
// Note: SendProxyByteScanner imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyByteScanner returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyByteScanner(out chan<- io.ByteScanner) chan<- io.ByteScanner {
	proxy := make(chan io.ByteScanner, ByteScannerCAP)
	go func() {
		n := ByteScannerQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.ByteScanner
		var e io.ByteScanner
		for {
			c = out
			if first == last {
