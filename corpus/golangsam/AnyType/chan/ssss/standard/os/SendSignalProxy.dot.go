...
// Note: SendProxySignal imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"os"
)
...
// SendProxySignal returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxySignal(out chan<- os.Signal) chan<- os.Signal {
	proxy := make(chan os.Signal, SignalCAP)
	go func() {
		n := SignalQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
