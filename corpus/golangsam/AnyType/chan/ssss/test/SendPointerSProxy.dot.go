...
// Note: SendProxyPointerS imports "container/ring" for the expanding buffer.
import (
	"container/ring"
)
...
// SendProxyPointerS returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyPointerS(out chan<- []*SomeType) chan<- []*SomeType {
	proxy := make(chan []*SomeType, PointerSCAP)
	go func() {
		n := PointerSQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- []*SomeType
		var e []*SomeType
