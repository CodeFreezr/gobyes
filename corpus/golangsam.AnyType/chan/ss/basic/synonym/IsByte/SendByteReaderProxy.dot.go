...
// Note: SendProxyByteReader imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyByteReader returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyByteReader(out chan<- io.ByteReader) chan<- io.ByteReader {
	proxy := make(chan io.ByteReader, ByteReaderCAP)
	go func() {
		n := ByteReaderQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.ByteReader
		var e io.ByteReader
		for {
			c = out
			if first == last {
