...
// Note: SendProxyByte imports "container/ring" for the expanding buffer.
import (
	"container/ring"
)
...
// SendProxyByte returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyByte(out chan<- byte) chan<- byte {
	proxy := make(chan byte, ByteCAP)
	go func() {
		n := ByteQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- byte
		var e byte
