...
// Note: SendProxySplitFunc imports "container/ring" for the expanding buffer.
import (
	bufio "bufio"
	"container/ring"
)
...
// SendProxySplitFunc returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxySplitFunc(out chan<- bufio.SplitFunc) chan<- bufio.SplitFunc {
	proxy := make(chan bufio.SplitFunc, SplitFuncCAP)
	go func() {
		n := SplitFuncQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- bufio.SplitFunc
		var e bufio.SplitFunc
		for {
			c = out
			if first == last {
