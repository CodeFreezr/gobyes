...
// Note: SendProxyCloser imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyCloser returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyCloser(out chan<- io.Closer) chan<- io.Closer {
	proxy := make(chan io.Closer, CloserCAP)
	go func() {
		n := CloserQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.Closer
		var e io.Closer
		for {
			c = out
			if first == last {
