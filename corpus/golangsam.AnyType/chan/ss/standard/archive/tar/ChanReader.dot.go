...
func sendReader(out chan<- *tar.Reader, inp ...*tar.Reader) {
	defer close(out)
	for i := range inp {
		out <- inp[i]
	}
...
func sendReaderSlice(out chan<- *tar.Reader, inp ...[]*tar.Reader) {
	defer close(out)
	for i := range inp {
		for j := range inp[i] {
			out <- inp[i][j]
...
func chanReaderFuncNok(out chan<- *tar.Reader, act func() (*tar.Reader, bool)) {
	defer close(out)
	for {
...
func chanReaderFuncErr(out chan<- *tar.Reader, act func() (*tar.Reader, error)) {
	defer close(out)
	for {
...
func joinReader(done chan<- struct{}, out chan<- *tar.Reader, inp ...*tar.Reader) {
	defer close(done)
	for i := range inp {
		out <- inp[i]
	}
...
func joinReaderSlice(done chan<- struct{}, out chan<- *tar.Reader, inp ...[]*tar.Reader) {
	defer close(done)
	for i := range inp {
		for j := range inp[i] {
			out <- inp[i][j]
...
func joinReaderChan(done chan<- struct{}, out chan<- *tar.Reader, inp <-chan *tar.Reader) {
	defer close(done)
	for i := range inp {
		out <- i
	}
...
func doitReader(done chan<- struct{}, inp <-chan *tar.Reader) {
	defer close(done)
	for i := range inp {
		_ = i // Drain inp
	}
...
func doitReaderSlice(done chan<- ([]*tar.Reader), inp <-chan *tar.Reader) {
	defer close(done)
	ReaderS := []*tar.Reader{}
	for i := range inp {
...
func doitReaderFunc(done chan<- struct{}, inp <-chan *tar.Reader, act func(a *tar.Reader)) {
	defer close(done)
	for i := range inp {
		act(i) // Apply action
	}
...
func pipeReaderBuffer(out chan<- *tar.Reader, inp <-chan *tar.Reader) {
	defer close(out)
	for i := range inp {
		out <- i
	}
...
func pipeReaderFunc(out chan<- *tar.Reader, inp <-chan *tar.Reader, act func(a *tar.Reader) *tar.Reader) {
	defer close(out)
	for i := range inp {
		out <- act(i)
	}
...
func pipeReaderFork(out1, out2 chan<- *tar.Reader, inp <-chan *tar.Reader) {
	defer close(out1)
	defer close(out2)
	for i := range inp {
		out1 <- i
