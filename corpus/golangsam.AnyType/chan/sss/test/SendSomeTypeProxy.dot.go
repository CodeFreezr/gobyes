...
// Note: SendProxySomeType imports "container/ring" for the expanding buffer.
import (
	"container/ring"
)
...
// SendProxySomeType returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxySomeType(out chan<- SomeType) chan<- SomeType {
	proxy := make(chan SomeType, SomeTypeCAP)
	go func() {
		n := SomeTypeQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- SomeType
		var e SomeType
