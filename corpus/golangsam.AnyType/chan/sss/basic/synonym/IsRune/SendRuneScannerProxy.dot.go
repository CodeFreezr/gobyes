...
// Note: SendProxyRuneScanner imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyRuneScanner returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyRuneScanner(out chan<- io.RuneScanner) chan<- io.RuneScanner {
	proxy := make(chan io.RuneScanner, RuneScannerCAP)
	go func() {
		n := RuneScannerQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.RuneScanner
		var e io.RuneScanner
		for {
			c = out
			if first == last {
