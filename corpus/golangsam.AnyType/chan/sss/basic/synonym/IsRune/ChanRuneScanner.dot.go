...
func ChanRuneScanner(inp ...io.RuneScanner) (out <-chan io.RuneScanner) {
	...
	go func(out chan<- io.RuneScanner, inp ...io.RuneScanner) {
		defer close(out)
		for i := range inp {
			out <- inp[i]
		}
...
func ChanRuneScannerFuncNok(act func() (io.RuneScanner, bool)) (out <-chan io.RuneScanner) {
	cha := make(chan io.RuneScanner)
	go func(out chan<- io.RuneScanner, act func() (io.RuneScanner, bool)) {
		defer close(out)
		for {
...
func ChanRuneScannerFuncErr(act func() (io.RuneScanner, error)) (out <-chan io.RuneScanner) {
	cha := make(chan io.RuneScanner)
	go func(out chan<- io.RuneScanner, act func() (io.RuneScanner, error)) {
		defer close(out)
		for {
...
func JoinRuneScanner(out chan<- io.RuneScanner, inp ...io.RuneScanner) (done <-chan struct{}) {
	...
	go func(done chan<- struct{}, out chan<- io.RuneScanner, inp ...io.RuneScanner) {
		defer close(done)
		for i := range inp {
			out <- inp[i]
		}
...
func JoinRuneScannerChan(out chan<- io.RuneScanner, inp <-chan io.RuneScanner) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, out chan<- io.RuneScanner, inp <-chan io.RuneScanner) {
		defer close(done)
		for i := range inp {
			out <- i
		}
...
func DoneRuneScanner(inp <-chan io.RuneScanner) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, inp <-chan io.RuneScanner) {
		defer close(done)
		for i := range inp {
			_ = i // Drain inp
		}
...
func DoneRuneScannerSlice(inp <-chan io.RuneScanner) (done <-chan []io.RuneScanner) {
	cha := make(chan []io.RuneScanner)
	go func(inp <-chan io.RuneScanner, done chan<- []io.RuneScanner) {
		defer close(done)
...
func DoneRuneScannerFunc(inp <-chan io.RuneScanner, act func(a io.RuneScanner)) (out <-chan struct{}) {
	...
	go func(done chan<- struct{}, inp <-chan io.RuneScanner, act func(a io.RuneScanner)) {
		defer close(done)
		for i := range inp {
			act(i) // Apply action
		}
...
func PipeRuneScannerBuffer(inp <-chan io.RuneScanner, cap int) (out <-chan io.RuneScanner) {
	...
	go func(out chan<- io.RuneScanner, inp <-chan io.RuneScanner) {
		defer close(out)
		for i := range inp {
			out <- i
		}
...
func PipeRuneScannerFunc(inp <-chan io.RuneScanner, act func(a io.RuneScanner) io.RuneScanner) (out <-chan io.RuneScanner) {
	...
	go func(out chan<- io.RuneScanner, inp <-chan io.RuneScanner, act func(a io.RuneScanner) io.RuneScanner) {
		defer close(out)
		for i := range inp {
			out <- act(i)
		}
...
func PipeRuneScannerFork(inp <-chan io.RuneScanner) (out1, out2 <-chan io.RuneScanner) {
	...
	go func(out1, out2 chan<- io.RuneScanner, inp <-chan io.RuneScanner) {
		defer close(out1)
		defer close(out2)
		for i := range inp {
			out1 <- i
