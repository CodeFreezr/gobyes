...
// Note: SendProxyBuffer imports "container/ring" for the expanding buffer.
import (
	"bytes"
	"container/ring"
)
...
// SendProxyBuffer returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyBuffer(out chan<- bytes.Buffer) chan<- bytes.Buffer {
	proxy := make(chan bytes.Buffer, BufferCAP)
	go func() {
		n := BufferQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
