...
// Note: SendProxyBoolS imports "container/ring" for the expanding buffer.
import (
	"container/ring"
)
...
// SendProxyBoolS returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyBoolS(out chan<- []bool) chan<- []bool {
	proxy := make(chan []bool, BoolSCAP)
	go func() {
		n := BoolSQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- []bool
		var e []bool
