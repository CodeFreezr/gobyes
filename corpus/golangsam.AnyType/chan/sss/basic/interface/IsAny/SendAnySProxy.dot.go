...
// Note: SendProxyAnyS imports "container/ring" for the expanding buffer.
import (
	"container/ring"
)
...
// SendProxyAnyS returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyAnyS(out chan<- []interface{}) chan<- []interface{} {
	proxy := make(chan []interface{}, AnySCAP)
	go func() {
		n := AnySQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
