...
// Note: SendProxyWriterAt imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyWriterAt returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyWriterAt(out chan<- io.WriterAt) chan<- io.WriterAt {
	proxy := make(chan io.WriterAt, WriterAtCAP)
	go func() {
		n := WriterAtQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.WriterAt
		var e io.WriterAt
		for {
			c = out
			if first == last {
