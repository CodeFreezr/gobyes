...
// Note: SendProxyWriteCloser imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyWriteCloser returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyWriteCloser(out chan<- io.WriteCloser) chan<- io.WriteCloser {
	proxy := make(chan io.WriteCloser, WriteCloserCAP)
	go func() {
		n := WriteCloserQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.WriteCloser
