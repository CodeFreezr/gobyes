...
// Note: SendProxySeeker imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
func SendProxySeeker(out chan<- io.Seeker) chan<- io.Seeker {
	proxy := make(chan io.Seeker, SeekerCAP)
	go func() {
		n := SeekerQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.Seeker
		var e io.Seeker
		for {
			c = out
			if first == last {
