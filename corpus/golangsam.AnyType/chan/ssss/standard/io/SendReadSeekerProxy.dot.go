...
// Note: SendProxyReadSeeker imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyReadSeeker returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyReadSeeker(out chan<- io.ReadSeeker) chan<- io.ReadSeeker {
	proxy := make(chan io.ReadSeeker, ReadSeekerCAP)
	go func() {
		n := ReadSeekerQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.ReadSeeker
		var e io.ReadSeeker
		for {
			c = out
			if first == last {
