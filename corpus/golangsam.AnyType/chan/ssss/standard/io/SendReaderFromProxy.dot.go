...
// Note: SendProxyReaderFrom imports "container/ring" for the expanding buffer.
import (
	"container/ring"
	"io"
)
...
// SendProxyReaderFrom returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyReaderFrom(out chan<- io.ReaderFrom) chan<- io.ReaderFrom {
	proxy := make(chan io.ReaderFrom, ReaderFromCAP)
	go func() {
		n := ReaderFromQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- io.ReaderFrom
		var e io.ReaderFrom
		for {
			c = out
			if first == last {
