...
// Note: SendProxyInt64 imports "container/ring" for the expanding buffer.
import (
	"container/ring"
)
...
// SendProxyInt64 returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyInt64(out chan<- int64) chan<- int64 {
	proxy := make(chan int64, Int64CAP)
	go func() {
		n := Int64QUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- int64
		var e int64
