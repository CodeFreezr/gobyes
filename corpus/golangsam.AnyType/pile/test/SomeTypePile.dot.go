...
// SomeTypePile is a hybrid container for
...
// p.Next() keeps traverses the SomeTypePile.
//
...
// Pile appends an `SomeType` item to the SomeTypePile.
//
...
func (d *SomeTypePile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *SomeTypePile) Iter() (item SomeType, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of SomeType) once the pile is closed.
//
...
func (d *SomeTypePile) Done() (done <-chan []SomeType) {
	cha := make(chan []SomeType)
	go func(cha chan<- []SomeType, d *SomeTypePile) {
		defer close(cha)
		d.offset = 0
