...
// PointerPile is a hybrid container for
...
// p.Next() keeps traverses the PointerPile.
//
...
func (d *PointerPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *PointerPile) Iter() (item *SomeType, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *PointerPile) Next() (item *SomeType, ok bool) {
...
// Done returns a channel which emits the result (as slice of Pointer) once the pile is closed.
//
...
func (d *PointerPile) Done() (done <-chan []*SomeType) {
	cha := make(chan []*SomeType)
	go func(cha chan<- []*SomeType, d *PointerPile) {
		defer close(cha)
		d.offset = 0
