...
// ReadWriterPile is a hybrid container for
// a lazily and concurrently populated growing-only slice
// of items (of type `*bufio.ReadWriter`)
...
// p.Next() keeps traverses the ReadWriterPile.
//
...
type ReadWriterPile struct {
	pile   chan *bufio.ReadWriter // channel to receive further items
	list   []*bufio.ReadWriter    // list of known items
	offset int                    // index for Next()
}

// MakeReadWriterPile returns a (pointer to a) fresh pile
// of items (of type `*bufio.ReadWriter`)
...
func MakeReadWriterPile(size, buff int) *ReadWriterPile {
	...
	pile.list = make([]*bufio.ReadWriter, 0, size)
	pile.pile = make(chan *bufio.ReadWriter, buff)
	return pile
}
...
func (d *ReadWriterPile) Done() (done <-chan []*bufio.ReadWriter) {
	...
	go func(cha chan<- []*bufio.ReadWriter, d *ReadWriterPile) {
		defer close(cha)
		d.offset = 0
