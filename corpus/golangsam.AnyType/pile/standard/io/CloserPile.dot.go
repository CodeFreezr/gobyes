...
// CloserPile is a hybrid container for
// a lazily and concurrently populated growing-only slice
// of items (of type `io.Closer`)
...
// p.Next() keeps traverses the CloserPile.
//
...
type CloserPile struct {
	pile   chan io.Closer // channel to receive further items
	list   []io.Closer    // list of known items
	offset int            // index for Next()
}

// MakeCloserPile returns a (pointer to a) fresh pile
// of items (of type `io.Closer`)
...
func MakeCloserPile(size, buff int) *CloserPile {
	pile := new(CloserPile)
	pile.list = make([]io.Closer, 0, size)
	pile.pile = make(chan io.Closer, buff)
	return pile
}
...
func (d *CloserPile) Done() (done <-chan []io.Closer) {
	...
	go func(cha chan<- []io.Closer, d *CloserPile) {
		defer close(cha)
		d.offset = 0
