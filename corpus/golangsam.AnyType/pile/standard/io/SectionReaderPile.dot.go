...
// SectionReaderPile is a hybrid container for
// a lazily and concurrently populated growing-only slice
// of items (of type `*io.SectionReader`)
...
// p.Next() keeps traverses the SectionReaderPile.
//
...
type SectionReaderPile struct {
	pile   chan *io.SectionReader // channel to receive further items
	list   []*io.SectionReader    // list of known items
	offset int                    // index for Next()
}

// MakeSectionReaderPile returns a (pointer to a) fresh pile
// of items (of type `*io.SectionReader`)
...
func MakeSectionReaderPile(size, buff int) *SectionReaderPile {
	pile := new(SectionReaderPile)
	pile.list = make([]*io.SectionReader, 0, size)
	pile.pile = make(chan *io.SectionReader, buff)
	return pile
}
...
func (d *SectionReaderPile) Done() (done <-chan []*io.SectionReader) {
	...
	go func(cha chan<- []*io.SectionReader, d *SectionReaderPile) {
		defer close(cha)
		d.offset = 0
