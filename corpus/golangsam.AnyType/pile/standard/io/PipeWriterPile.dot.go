...
// PipeWriterPile is a hybrid container for
...
// p.Next() keeps traverses the PipeWriterPile.
//
...
// MakePipeWriterPile returns a (pointer to a) fresh pile
// of items (of type `*io.PipeWriter`)
...
func (d *PipeWriterPile) Done() (done <-chan []*io.PipeWriter) {
	...
	go func(cha chan<- []*io.PipeWriter, d *PipeWriterPile) {
		defer close(cha)
		d.offset = 0
