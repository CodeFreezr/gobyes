...
// ReadWriteCloserPile is a hybrid container for
// a lazily and concurrently populated growing-only slice
// of items (of type `io.ReadWriteCloser`)
...
// p.Next() keeps traverses the ReadWriteCloserPile.
//
...
type ReadWriteCloserPile struct {
	pile   chan io.ReadWriteCloser // channel to receive further items
	list   []io.ReadWriteCloser    // list of known items
	offset int                     // index for Next()
}

// MakeReadWriteCloserPile returns a (pointer to a) fresh pile
// of items (of type `io.ReadWriteCloser`)
...
func MakeReadWriteCloserPile(size, buff int) *ReadWriteCloserPile {
	pile := new(ReadWriteCloserPile)
	pile.list = make([]io.ReadWriteCloser, 0, size)
	pile.pile = make(chan io.ReadWriteCloser, buff)
	return pile
}
...
func (d *ReadWriteCloserPile) Done() (done <-chan []io.ReadWriteCloser) {
	...
	go func(cha chan<- []io.ReadWriteCloser, d *ReadWriteCloserPile) {
		defer close(cha)
		d.offset = 0
