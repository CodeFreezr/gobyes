...
// ReadCloserPile is a hybrid container for
// a lazily and concurrently populated growing-only slice
// of items (of type `io.ReadCloser`)
...
// p.Next() keeps traverses the ReadCloserPile.
//
...
type ReadCloserPile struct {
	pile   chan io.ReadCloser // channel to receive further items
	list   []io.ReadCloser    // list of known items
	offset int                // index for Next()
}

// MakeReadCloserPile returns a (pointer to a) fresh pile
// of items (of type `io.ReadCloser`)
...
func MakeReadCloserPile(size, buff int) *ReadCloserPile {
	...
	pile.list = make([]io.ReadCloser, 0, size)
	pile.pile = make(chan io.ReadCloser, buff)
	return pile
}
...
func (d *ReadCloserPile) Done() (done <-chan []io.ReadCloser) {
	...
	go func(cha chan<- []io.ReadCloser, d *ReadCloserPile) {
		defer close(cha)
		d.offset = 0
