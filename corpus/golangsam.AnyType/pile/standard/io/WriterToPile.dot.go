...
// WriterToPile is a hybrid container for
// a lazily and concurrently populated growing-only slice
// of items (of type `io.WriterTo`)
...
// p.Next() keeps traverses the WriterToPile.
//
...
type WriterToPile struct {
	pile   chan io.WriterTo // channel to receive further items
	list   []io.WriterTo    // list of known items
	offset int              // index for Next()
}

// MakeWriterToPile returns a (pointer to a) fresh pile
// of items (of type `io.WriterTo`)
...
func MakeWriterToPile(size, buff int) *WriterToPile {
	pile := new(WriterToPile)
	pile.list = make([]io.WriterTo, 0, size)
	pile.pile = make(chan io.WriterTo, buff)
	return pile
}
...
func (d *WriterToPile) Done() (done <-chan []io.WriterTo) {
	...
	go func(cha chan<- []io.WriterTo, d *WriterToPile) {
		defer close(cha)
		d.offset = 0
