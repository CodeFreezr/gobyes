...
// FilePile is a hybrid container for
// a lazily and concurrently populated growing-only slice
// of items (of type `*os.File`)
...
// p.Next() keeps traverses the FilePile.
//
...
type FilePile struct {
	pile   chan *os.File // channel to receive further items
	list   []*os.File    // list of known items
	offset int           // index for Next()
}

// MakeFilePile returns a (pointer to a) fresh pile
// of items (of type `*os.File`)
...
func MakeFilePile(size, buff int) *FilePile {
	pile := new(FilePile)
	pile.list = make([]*os.File, 0, size)
	pile.pile = make(chan *os.File, buff)
	return pile
}
...
func (d *FilePile) Done() (done <-chan []*os.File) {
	...
	go func(cha chan<- []*os.File, d *FilePile) {
		defer close(cha)
		d.offset = 0
