...
// BoolSPile is a hybrid container for
...
//
// Usage for a pile `p`:
//  p := MakeBoolSPile(128, 32)
//
// Have it grow concurrently using multiple:
//  var item []bool = something
//  p.Pile(item)
...
// p.Next() keeps traverses the BoolSPile.
//
...
// Pile appends an `[]bool` item to the BoolSPile.
//
...
func (d *BoolSPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *BoolSPile) Iter() (item []bool, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of BoolS) once the pile is closed.
//
...
func (d *BoolSPile) Done() (done <-chan [][]bool) {
	cha := make(chan [][]bool)
	go func(cha chan<- [][]bool, d *BoolSPile) {
		defer close(cha)
		d.offset = 0
