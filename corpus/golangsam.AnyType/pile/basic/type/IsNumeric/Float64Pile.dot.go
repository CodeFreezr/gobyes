...
// Float64Pile is a hybrid container for
...
// p.Next() keeps traverses the Float64Pile.
//
...
// Pile appends an `float64` item to the Float64Pile.
//
...
func (d *Float64Pile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *Float64Pile) Iter() (item float64, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of Float64) once the pile is closed.
//
...
func (d *Float64Pile) Done() (done <-chan []float64) {
	cha := make(chan []float64)
	go func(cha chan<- []float64, d *Float64Pile) {
		defer close(cha)
		d.offset = 0
