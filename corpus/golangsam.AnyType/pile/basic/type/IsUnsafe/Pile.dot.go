...
//
type Pile struct {
	pile   chan uintptr // channel to receive further items
	list   []uintptr    // list of known items
...
func MakePile(size, buff int) *Pile {
	pile := new(Pile)
	pile.list = make([]uintptr, 0, size)
	pile.pile = make(chan uintptr, buff)
	return pile
}

// Pile appends an `uintptr` item to the Pile.
//
// Note: Pile will block iff buff is exceeded and no Done() or Next()'s are used.
func (d *Pile) Pile(item uintptr) {
	d.pile <- item
}
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *Pile) Iter() (item uintptr, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *Pile) Next() (item uintptr, ok bool) {
...
func (d *Pile) Done() (done <-chan []uintptr) {
	cha := make(chan []uintptr)
	go func(cha chan<- []uintptr, d *Pile) {
		defer close(cha)
		d.offset = 0
