...
// ReplacerPile is a hybrid container for
...
//
// Usage for a pile `p`:
//  p := MakeReplacerPile(128, 32)
//
// Have it grow concurrently using multiple:
//  var item *strings.Replacer = something
//  p.Pile(item)
...
// p.Next() keeps traverses the ReplacerPile.
//
...
// Pile appends an `*strings.Replacer` item to the ReplacerPile.
//
...
func (d *ReplacerPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *ReplacerPile) Iter() (item *strings.Replacer, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of Replacer) once the pile is closed.
//
...
func (d *ReplacerPile) Done() (done <-chan []*strings.Replacer) {
	cha := make(chan []*strings.Replacer)
	go func(cha chan<- []*strings.Replacer, d *ReplacerPile) {
		defer close(cha)
		d.offset = 0
