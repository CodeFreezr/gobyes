...
// of items (of type `*strings.Reader`)
...
//  var item *strings.Reader = something
//  p.Pile(item)
...
type ReaderPile struct {
	pile   chan *strings.Reader // channel to receive further items
	list   []*strings.Reader    // list of known items
...
func MakeReaderPile(size, buff int) *ReaderPile {
	pile := new(ReaderPile)
	pile.list = make([]*strings.Reader, 0, size)
	pile.pile = make(chan *strings.Reader, buff)
	return pile
}

// Pile appends an `*strings.Reader` item to the ReaderPile.
//
// Note: Pile will block iff buff is exceeded and no Done() or Next()'s are used.
func (d *ReaderPile) Pile(item *strings.Reader) {
	d.pile <- item
}
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *ReaderPile) Iter() (item *strings.Reader, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *ReaderPile) Next() (item *strings.Reader, ok bool) {
...
func (d *ReaderPile) Done() (done <-chan []*strings.Reader) {
	cha := make(chan []*strings.Reader)
	go func(cha chan<- []*strings.Reader, d *ReaderPile) {
		defer close(cha)
		d.offset = 0
