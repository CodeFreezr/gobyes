...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *Pile) Iter() (item int, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *Pile) Next() (item int, ok bool) {
...
func (d *Pile) Done() (done <-chan []int) {
	cha := make(chan []int)
