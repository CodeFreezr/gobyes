...
// p.Next() keeps traverses the UIntPile.
//
...
//
type UIntPile struct {
	pile   chan uint // channel to receive further items
	list   []uint    // list of known items
	offset int       // index for Next()
}

// MakeUIntPile returns a (pointer to a) fresh pile
// of items (of type `uint`)
// with size as initial capacity
// and
// with buff as initial leeway, allowing as many Pile's to execute non-blocking before respective Done or Next's.
func MakeUIntPile(size, buff int) *UIntPile {
	pile := new(UIntPile)
	pile.list = make([]uint, 0, size)
	pile.pile = make(chan uint, buff)
	return pile
}

// Pile appends an `uint` item to the UIntPile.
//
// Note: Pile will block iff buff is exceeded and no Done() or Next()'s are used.
func (d *UIntPile) Pile(item uint) {
	d.pile <- item
}
...
func (d *UIntPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *UIntPile) Iter() (item uint, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *UIntPile) Next() (item uint, ok bool) {
...
// Done returns a channel which emits the result (as slice of UInt) once the pile is closed.
//
...
func (d *UIntPile) Done() (done <-chan []uint) {
	cha := make(chan []uint)
	go func(cha chan<- []uint, d *UIntPile) {
		defer close(cha)
		d.offset = 0
