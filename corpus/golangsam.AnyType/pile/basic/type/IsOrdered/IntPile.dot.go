...
// IntPile is a hybrid container for
...
//
// Usage for a pile `p`:
//  p := MakeIntPile(128, 32)
//
// Have it grow concurrently using multiple:
//  var item int = something
//  p.Pile(item)
...
// p.Next() keeps traverses the IntPile.
//
...
func (d *IntPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *IntPile) Iter() (item int, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of Int) once the pile is closed.
//
...
func (d *IntPile) Done() (done <-chan []int) {
	cha := make(chan []int)
