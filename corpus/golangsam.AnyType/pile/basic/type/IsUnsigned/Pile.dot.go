...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *Pile) Iter() (item uint, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *Pile) Next() (item uint, ok bool) {
...
func (d *Pile) Done() (done <-chan []uint) {
	cha := make(chan []uint)
	go func(cha chan<- []uint, d *Pile) {
		defer close(cha)
		d.offset = 0
