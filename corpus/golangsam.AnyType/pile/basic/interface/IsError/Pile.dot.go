...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *Pile) Iter() (item error, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *Pile) Next() (item error, ok bool) {
...
func (d *Pile) Done() (done <-chan []error) {
	cha := make(chan []error)
	go func(cha chan<- []error, d *Pile) {
		defer close(cha)
		d.offset = 0
