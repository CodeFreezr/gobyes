...
// AnySPile is a hybrid container for
...
// p.Next() keeps traverses the AnySPile.
//
...
// Pile appends an `[]interface{}` item to the AnySPile.
//
...
func (d *AnySPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *AnySPile) Iter() (item []interface{}, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *AnySPile) Done() (done <-chan [][]interface{}) {
	...
	go func(cha chan<- [][]interface{}, d *AnySPile) {
		defer close(cha)
		d.offset = 0
