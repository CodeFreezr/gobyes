...
//  var item bytes.Reader = something
//  p.Pile(item)
...
type ReaderPile struct {
	pile   chan bytes.Reader // channel to receive further items
	list   []bytes.Reader    // list of known items
...
func MakeReaderPile(size, buff int) *ReaderPile {
	pile := new(ReaderPile)
	pile.list = make([]bytes.Reader, 0, size)
	pile.pile = make(chan bytes.Reader, buff)
	return pile
}

// Pile appends an `bytes.Reader` item to the ReaderPile.
//
// Note: Pile will block iff buff is exceeded and no Done() or Next()'s are used.
func (d *ReaderPile) Pile(item bytes.Reader) {
	d.pile <- item
}
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *ReaderPile) Iter() (item bytes.Reader, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *ReaderPile) Next() (item bytes.Reader, ok bool) {
...
func (d *ReaderPile) Done() (done <-chan []bytes.Reader) {
	cha := make(chan []bytes.Reader)
	go func(cha chan<- []bytes.Reader, d *ReaderPile) {
		defer close(cha)
		d.offset = 0
