...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *Pile) Iter() (item []byte, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *Pile) Next() (item []byte, ok bool) {
...
func (d *Pile) Done() (done <-chan [][]byte) {
	cha := make(chan [][]byte)
	go func(cha chan<- [][]byte, d *Pile) {
		defer close(cha)
		d.offset = 0
