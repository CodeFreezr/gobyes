...
// p.Next() keeps traverses the BufferPile.
//
...
//
type BufferPile struct {
	pile   chan bytes.Buffer // channel to receive further items
	list   []bytes.Buffer    // list of known items
	offset int               // index for Next()
}

// MakeBufferPile returns a (pointer to a) fresh pile
// of items (of type `bytes.Buffer`)
// with size as initial capacity
// and
// with buff as initial leeway, allowing as many Pile's to execute non-blocking before respective Done or Next's.
func MakeBufferPile(size, buff int) *BufferPile {
	pile := new(BufferPile)
	pile.list = make([]bytes.Buffer, 0, size)
	pile.pile = make(chan bytes.Buffer, buff)
	return pile
}

// Pile appends an `bytes.Buffer` item to the BufferPile.
//
// Note: Pile will block iff buff is exceeded and no Done() or Next()'s are used.
func (d *BufferPile) Pile(item bytes.Buffer) {
	d.pile <- item
}
...
func (d *BufferPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *BufferPile) Iter() (item bytes.Buffer, ok bool) {
	d.offset = 0
	return d.Next()
}
...
func (d *BufferPile) Next() (item bytes.Buffer, ok bool) {
...
// Done returns a channel which emits the result (as slice of Buffer) once the pile is closed.
//
...
func (d *BufferPile) Done() (done <-chan []bytes.Buffer) {
	cha := make(chan []bytes.Buffer)
	go func(cha chan<- []bytes.Buffer, d *BufferPile) {
		defer close(cha)
		d.offset = 0
