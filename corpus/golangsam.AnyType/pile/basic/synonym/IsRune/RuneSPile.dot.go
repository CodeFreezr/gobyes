...
// RuneSPile is a hybrid container for
...
//
// Usage for a pile `p`:
//  p := MakeRuneSPile(128, 32)
//
// Have it grow concurrently using multiple:
//  var item []rune = something
//  p.Pile(item)
...
// p.Next() keeps traverses the RuneSPile.
//
...
// Pile appends an `[]rune` item to the RuneSPile.
//
...
func (d *RuneSPile) Close() (err error) {
	defer func() {
		if r := recover(); r != nil {
			var ok bool
...
// Usage for a pile `p`:
//  for item, ok := p.Iter(); ok; item, ok = p.Next() { ... do sth with item ... }
func (d *RuneSPile) Iter() (item []rune, ok bool) {
	d.offset = 0
	return d.Next()
}
...
// Done returns a channel which emits the result (as slice of RuneS) once the pile is closed.
//
...
func (d *RuneSPile) Done() (done <-chan [][]rune) {
	cha := make(chan [][]rune)
	go func(cha chan<- [][]rune, d *RuneSPile) {
		defer close(cha)
		d.offset = 0
